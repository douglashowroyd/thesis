...
\nomenclature[.]{$d(\cdot, \cdot)$}{distance between two points in a metric space}
\[
...
\nomenclature[.]{IFS}{iterated function system}
...
\nomenclature[.]{SSC}{strong separation condition}
...
It is a classical result due to \cite{hutchinson} that the Hausdorff dimension of a self-similar set satisfying the strong separation condition and with contractions $\left\{c_i \right\}_{i\in \lambda}$ is the unique $s$ satisfying $\sum_{i\in \Lambda} c_i^s = 1$. In fact, these sets are incredibly regular so the box, Assouad and lower dimensions are all equal to the Hausdorff dimension. As we weaken the separation condition these numbers start to differ in a number of cases. Studying such cases has seen huge advances recently, as in \cite{hochman-ss-w-overlaps}. The Hausdorff dimension of self-similar measures on these sets is known to be $\underline{\dim}_{\textup{H}} \mu = \sum p_i \log p_i / \log c_i$ and these measures are exact dimensional. \textcolor{red}{who showed this?}  
...
\nomenclature[.]{VSSC}{very strong separation condition}
...
We assume the self-affine sets studied here satisfy the \emph{very strong separation condition}, which was used by Olsen in \cite{sponges}. Again, this is the natural condition to assume in the context of regularity dimensions because without this assumption the self-affine measures tend not to be doubling, see \cite{doublingcarpets, f raser-howroyd}. \textcolor{red}{maybe elaborate a bit here}
...
\]
The Assouad dimension of these sets was first calculated by Mackay in \cite{mackay}, whilst the lower dimension is due to Fraser in \cite{fraser-carpets}, and they are
\[
...
Formulae for $\sup_x \overline{\dim}_{\text{loc}}(x,\mu)$ and $T(\mu)$ for the self-affine measures $\mu$ we consider in this section can be found in \cite{sponges}, where the notation $\overline{a}$ and $\overline{A}$ was used, respectively.    Also, the Assouad dimension of these sponges was first computed by Mackay \cite{mackay} in the case $d=2$ and in \cite{fraser-howroyd} for general $d$. \textcolor{red}{what is hausdorff dim of theses measures?}

\textcolor{red}{do lower dim changes below}
...
For any $\omega \in \mathcal{I}^{\mathbb{N}}$ with $x= \pi(\omega) \in F$ and $0<R\le 1$, by Proposition \ref{ch-upper-reg:ballscubes} we see that
\[
...
\textcolor{red}{this is where nl = nl+1 comes in}
